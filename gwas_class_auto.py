...
def impute_single_trait(dataframe: pd.DataFrame(), imputing_col: str , covariate_cols: list, groupby: list, scaler = StandardScaler(), imputer = SoftImpute(verbose = False)):
    ...
    #print(imputing_col, covariate_cols, groupby)
...
def fancy_display(df, download_name = 'default.csv'):
    ...
    return pn.Column(pn.Row(filename, button), download_table).save('phewas.html')
...
class gwas_pipe:
    ...
    def __init__(self, 
                 ...
                 founderfile: str = '/founder_genotypes/founders7.2',
                 gtca_path: str = '',
        ...
        # self.founder_genotypes = founderfile
        ...
        if not len(self.traits):
            try: 
                if len(temptraits := df.columns[df.columns.str.contains('regressedlr_')]):
                    ...
                    printwithlog(f'importing traits from {self.path}data_dict_{self.project_name}.csv')
                    tempdd = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/data_dict_{self.project_name}.csv')
                    ...
                    if len(_ :=  self.df.columns[~self.df.columns.isin(tempdd.measure)]):
                        printwithlog(f'traits in raw_data but not in data_dict: https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/missing_inRawDataNotInDatadic.txt')
                    ...
                    if len(_ := tempdd.measure[~tempdd.measure.isin(self.df.columns)]):
                        printwithlog(f'traits in data_dict but not in raw_data: https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/missing_inDatadicNotInRawData.txt')
                        _.to_frame().to_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/missing_inDatadicNotInRawData.txt',index = False, header = None)
                
            except:
                printwithlog(f'could not open https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/data_dict_{self.project_name}.csv, traits will be set to empty list')
                self.traits = []
        ...
        if not len(trait_descriptions):
            try: 
                printwithlog(f'importing trait descriptions from https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/data_dict_{self.project_name}.csv')
                trait_descriptions = get_trait_descriptions_f(pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/data_dict_{self.project_name}.csv'), traits)
            except:
                printwithlog(f'could not open https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/data_dict_{self.project_name}.csv, descriptions will be filled with "UNK"')
    ...
    def generateGRM(self, autosome_list: list = [], print_call: bool = True, extra_chrs: list = ['X', 'Y', 'MT'],  **kwards):
        '''
    ...
    def make_genetic_correlation_figure(self, order = 'cluster', traits= [], save = False):
            ...
            if order == 'cluster': 
                ...
                gp.loc[:,:] = 1- np.abs(gp.fillna(0).values + np.eye(len(gp)))**2
            ...
            fig = fig*hv.Labels(H2.assign(gtex = H2.g.map(lambda x: f"{int(x*100)}%")), kdims = ['trait', 'trait'],vdims=['gtex']).opts(text_font_size=f'{int(7*1.5*30/len(traits))}pt', text_color='black')
            fig = fig.opts(frame_height=900, frame_width=900,title = f'Genetic correlation', xlabel = '', ylabel = '',
                           fontsize={ 'xticks': f'{int(7*1.5*30/len(traits))}pt', 'yticks': f'{int(7*1.5*30/len(traits))}pt'},
    ...
    def effectsize(self, qtltable: pd.DataFrame() = '', display_plots: bool = True):
        ...
        if len(qtltable) == 0: qtltable = pd.read_csv(f'{self.path}results/qtls/finalqtl.csv')
    ...
    def locuszoom(self, qtltable: pd.DataFrame() = '',  qtl_r2_thresh: float = .65, save = True,
        ...
        if not hasattr(self, 'gtf'): self.gtf = self.get_ncbi_gtf()
        ...
        if not skip_ld_calculation:
            ...
            for name, row in tqdm(list(out.iterrows())):
                ...
                tempdf.to_csv( f'{self.path}results/lz/lzplottable_{row.trait}_{row.SNP}.tsv', sep = '\t')
...
####################################
        ...
        #     st.markdown(f':green[Running original locuszoom...]')
        for num, (_, qtl_row) in tqdm(list(enumerate(out.iterrows()))):
            ...
            try:test = pd.read_csv(path, 
                               dtype = {'p':float, 'bp': int, 'R2': float, 'DP': float}, sep = '\t',
                                   na_values =  na_values_4_pandas)\
                         .replace([np.inf, -np.inf], np.nan)\
                         .dropna(how = 'any', subset = ['Freq','b','se','p','R2','DP'])
            except: raise Exception(f"couldn't open this file {self.path}results/lz/lzplottable_{qtl_row.trait}_{qtl_row.SNP}.tsv") 
    ...
    def get_ncbi_gtf(self, extractall = False):
        ...
        status = st.status('Reading gene list from NCBI RefSeq from NCBI GTF...', state='running')
        # placeholder = st.empty()
        # with placeholder.container():
        #     st.markdown(':green[Reading gene list from NCBI RefSeq from NCBI GTF...]')
    ...
    def locuszoom_interactive(self, qtltable:pd.DataFrame() = '', qtl_r2_thresh: float = .8,  padding: float = 1e5):
        ...
        if type(qtltable) != pd.core.frame.DataFrame:
            qtltable = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/qtls/finalqtl.csv')
        if not hasattr(self, 'gtf'): self.gtf = self.get_ncbi_gtf()
        gtf = self.gtf.query('biotype != "transcript"')
        def glk(gene):
        ...
        for _, topsnp in tqdm(list(qtltable.reset_index().iterrows())):
            ...
            path = f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/lz/lzplottable%40{topsnp.trait}%40{topsnp.SNP}.tsv'
            printwithlog(f'file path {path}...')
            data = pd.read_csv(path, sep = '\t').query('p != "UNK"')
            ...
            causal = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/qtls/possible_causal_snps.tsv' , sep = '\t')\
                       .query(f'SNP_qtl == "{topsnp.SNP}"')
            causal['bp'] = causal.SNP.str.extract(':(\d+)')
            causal = causal.merge(data[['SNP', '-log10(P)']], on='SNP')

            phewas = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/phewas/pretty_table_both_match.tsv' , sep = '\t')\
                       .query(f'SNP_QTL == "{topsnp.SNP}"')
            phewas['bp'] = phewas.SNP_PheDb.str.extract(':(\d+)')
            phewas = phewas.merge(data[['SNP', '-log10(P)']], left_on='SNP_PheDb', right_on='SNP')

            eqtl = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/eqtl/pretty_eqtl_table.csv' )
            ...
            sqtl = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/sqtl/pretty_sqtl_table.csv' )
            ...
            fig.update_layout(template='simple_white',width = 950, height = 700, coloraxis_colorbar_x=1.05,
            ...
            # fig.write_html(f'lz.html'.replace(':', '_'))
            # with open("lz.html", 'r', encoding='utf-8') as file:
            #     st.components.v1.html(file.read(), width=1000, height=800, scrolling=True)
    ...
    def get_closest_snp(self, s, include_snps_in_gene = False, include_snps_in_ld  = False):
        if not hasattr(self, 'gtf'): self.gtf = self.get_ncbi_gtf()
        gtff = self.gtf.query("biotype == 'transcript' and gene != ''")
        status = st.status(f'Getting SNPs...', state='running')
    ...
    def locuszoom_widget(self):
        if not hasattr(self, 'gtf'): self.gtf = self.get_ncbi_gtf()
        cloc = st.selectbox(label="Choose gene or position",options=tuple(self.gtf.gene.unique()), \
                                placeholder='Gene or pos:',index=None, key='lz_gene')
        ctrait = st.selectbox(label ='Trait:',options=[x.replace('regressedlr_', '') for x in self.traits],placeholder='Choose a trait', index=None, key = 'lz_trait')
        cqtl_r2_thresh = st.number_input(label = 'R2 boundary', value = .65, max_value = 1.0, min_value = 0.0, step = 0.01, key='lz_r2') 
        cpadding = st.number_input(label = 'Padding', value = int(.1e6), max_value = int(10e6), min_value = 0,format='%d')
        def locuszoom_manual_query(loc, trait,qtl_r2_thresh, padding):
            aa = self.get_closest_snp(loc).loc[0, :]
            tempqtls = pd.read_csv(f'https://palmerlab.s3.sdsc.edu/tsanches_dash_genotypes/gwas_results/{self.project_name}/results/gwas/regressedlr_{trait}_chrgwas{aa.Chr}.mlma', sep = '\t', index_col = 1).loc[aa.SNP].to_frame().T.reset_index(names = 'SNP')
        ...
        if cloc is not None and ctrait is not None:
            if st.button("Run Locuszoom", type='primary'):
                locuszoom_manual_query(loc = cloc, trait = ctrait, qtl_r2_thresh= cqtl_r2_thresh, padding = cpadding)
                with open(f'locuszoom.html', 'r', encoding='utf-8') as file:
                    st.components.v1.html(file.read(), width=None, height=700, scrolling=True)
        return 
    
    def phewas(self, qtltable: pd.DataFrame() = '', phewas_file = '',
        ...
        status = st.status(f'Starting PheWAS for project {self.project_name}', state='running')
        ...
        #      self.plink(bfile = self.genotypes_subset, 
        #                 chr = row.Chr, 
        #                 r2 = 'dprime',
        #                 ld_snp = row.name, 
        #                 ld_window_r2 = 0.00001,
        ...
        nearby_snps = pd.DataFrame()
        status.update(label='Running PheWAS...', state='running')
        for  _, row in stqdm(list(qtltable.iterrows())):
            nearby_snps = pd.concat([nearby_snps, 
                        self.plink(bfile = self.genotypes_subset, 
                        chr = row.Chr, 
                        r2 = 'dprime',
                        ld_snp = row.name, 
        ...
        status.update(label='Cleaning up...🐀', state='running')
        ...
        if annotate:
            status.update(label='Annotating...', state='running')
    ...
    def phewas_widget(self):
        ...
        cloc = st.selectbox(label="Choose Gene or position",options=tuple(self.gtf.gene.unique()), \
                                placeholder='gene or pos:',index=None, key='phewas_gene')
        phewas_file = st.selectbox(label='Choose parquet phewas file: ',
                                       options=[self.phewas_db],
                                       placeholder='phewas_file:',index=None, key='phewas_file')
        cqtl_r2_thresh = st.number_input(label = 'R2 boundary', value = .9, max_value = 1.0, min_value = 0.0, step = 0.01, key='phewas_r2') 
        def phewas_manual_query(loc,qtl_r2_thresh, phewas_file):
            ...
            # st.dataframe(self.phewas_widget_result)
            pn.extension('tabulator')
            bsname = basename(phewas_file).split('.')[0]
            fancy_display(self.phewas_widget_result, download_name = f'phewas_result_{loc}_{bsname}.csv')
            #print('Done! result is in self.phewas_widget_result')
            return 
        
        if cloc is not None and phewas_file is not None:
            if st.button("Run PheWAS", type='primary'):
                phewas_manual_query(cloc, cqtl_r2_thresh, phewas_file)
                with open("phewas.html", 'r', encoding='utf-8') as file:
                    st.components.v1.html(file.read(), width=None, height=500, scrolling=True)
        #interact_manual(phewas_manual_query,loc = cloc, qtl_r2_thresh= cqtl_r2_thresh, phewas_file=phewas_file)
        return 
        
    def eQTL(self, qtltable: pd.DataFrame()= '',
        ...
        if not len(qtltable): 
            printwithlog(f'reading file from {self.path}results/qtls/finalqtl.csv...') 
            qtltable = pd.read_csv(f'{self.path}results/qtls/finalqtl.csv').set_index('SNP').loc[:, : 'significance_level']
        out = []
        for tissue in tqdm(tissue_list,  position=0, desc="tissue", leave=True):

            tempdf = pd.read_csv(f'https://ratgtex.org/data/eqtl/{tissue}.{self.genome}.cis_qtl_signif.txt.gz', sep = '\t').assign(tissue = tissue)\
        ...
        gene_conv_table = pd.DataFrame([(x['query'], defaultdict(lambda: '', x)['symbol']) for x \
                                in  mg.querymany(eqtl_info.gene_id , scopes='ensembl.gene', fields='symbol', species=mygene_species, verbose = False, silent = True)],
    ...
    def sQTL(self, qtltable: pd.DataFrame() = '',
        ...
        for tissue, (typ, prefix) in tqdm(list(itertools.product(tissue_list, loop_str)),  position=0, desc="tissue+CisTrans", leave=True):
            ...
            tempdf = pd.read_csv(f'https://ratgtex.org/data/splice/{tissue}.{self.genome}.splice.{prefix}.txt.gz', sep = '\t').assign(tissue = tissue)\
        ...
        gene_conv_table = pd.DataFrame([(x['query'], defaultdict(lambda: '', x)['symbol']) for x \
                                in  mg.querymany(sqtl_info.gene_id , scopes='ensembl.gene', fields='symbol', species=mygene_species, verbose = False, silent = True)],
                              columns = ['gene_id','Ensembl_gene'])
        gene_conv_table= gene_conv_table.groupby('gene_id').agg(lambda df: ' | '.join(df.drop_duplicates())).reset_index()
        ...
        for num, t in tqdm(list(enumerate(traitlist))):
            ...
            for opt in [f'regressedlr_{t.replace("regressedlr_", "")}.loco.mlma', 
                ...
                if glob(f'{self.path}results/gwas/{opt}'):
                    ...
                    pass
                    #printwithlog(f'could not open file {self.path}results/gwas/{opt}')
            if len(df_gwas) == 0 :
    ...
    def porcupineplotv2(self, qtltable = '', traitlist: list = [], display_figure = False, skip_manhattan = False, maxtraits = 60):
        printwithlog('starting porcopine plot v2')
    ...
    def qqplot(self, col2use = 'trait', add_pval_thresh = True, save = True):
        ...
        qtltable = pd.read_csv(f'{self.path}results/qtls/finalqtlannotated.csv').query('QTL == True').reset_index().drop(['rank', 'p'], axis = 1)\
                      .merge(gwas_res[['SNP', 'trait', 'rank', 'p', 'prefix']], on = ['SNP', 'trait'], how = 'left', suffixes=['', '2drop'])
        
        from sklearn.preprocessing import MinMaxScaler
        def readsample(tempt, minp = 0):
            #temp = dd.read_csv(tempt+'/*.mlma', sep = '\t',usecols=['SNP', 'p'], dtype={'SNP':str, 'p':float}).compute().assign(trait = tempt.split('/')[-1])
            temp = pd.concat(pd.read_csv(x, sep = '\t',usecols=['SNP', 'p'], dtype={'SNP':str, 'p':float}).assign(trait = tempt.split('/')[-1]) for x in glob(tempt+'/*.mlma'))
            ...
            return temp.reset_index(drop = True)#.to_json()
        ...
        if add_pval_thresh == True:
            #dynspread(, max_px=4,threshold= 1)
            fig = fig*datashade(hv.Points(subsample, kdims = ['rnk','p']), pixel_ratio= .5, aggregator=ds.count(), min_alpha=1, \
                                            width = 800,height = 800, y_range= yrange,x_range= xrange,  cmap = sns.dark_palette(mplrgb2hex([.9,.9,.9]), reverse=True, as_cmap=True), 
        ...
        for idx, (trait, df) in tqdm(enumerate(gwas_res.loc[crit].groupby(col2use))):
            fig = fig*dynspread(datashade( hv.Points(df, kdims = ['rank','p']).opts(color=d[trait], size=0), pixel_ratio= 1, aggregator=ds.count(),
                                          width=800, height=800, y_range= yrange,x_range= xrange, \
        ...
        fig.opts( shared_axes=False, frame_width=800, frame_height=800, title = f'Q-Q plot',show_legend=True, legend_position='right', xlabel = 'expected', ylabel = 'observed')
        if save:
    ...
    def report(self, round_version: str = '10.2', covariate_explained_var_threshold: float = 0.02, gwas_version='current', sorted_gcorr = False, add_gwas_latent_space= 'nmf'):
        ...
        if round(self.threshold, 2) == round(self.threshold05, 2):
            threshtext = f'''* {self.genome}:{round_version} 5%: {round(self.threshold, 2)}'''
        else:
            threshtext = f'''* {self.genome}:{round_version} 10%: {round(self.threshold, 2)}
* {self.genome}:{round_version} 5% : {round(self.threshold05, 2)}'''
...
{threshtext}"""
        ...
        if os.path.isfile(f'{self.path}missing_rfid_list.txt'): 
            trait_d_card += [pn.Card(fancy_display(pd.read_csv(f'{self.path}missing_rfid_list.txt', dtype = str, header = None, names=['Unmapped RFIDs']), 'missing_rats.csv'), title = 'Unmapped Rats', collapsed=True)]
...
For the figure, the upper triangle represents the genetic correlation (ranges from [-1:1]), while the lower triangle represents the phenotypic correlation. Meanwhile the diagonal displays the heritability (ranges from [0:1]) of the traits. Hierarchical clustering is performed using [scipy's linkage function](https://docs.scipy.org/doc/scipy/reference/generated/scipy.cluster.hierarchy.linkage.html) with the genetic correlation. Dendrogram is drawn using [scipy dendrogram](https://docs.scipy.org/doc/scipy/reference/generated/scipy.cluster.hierarchy.dendrogram.html) where color coding for clusters depends on a distance threshold set to 70% of the maximum linkage distance. Asterisks means that test failed, for genetic relationship the main failure point is if the 2 traits being tested are colinear, while for the phenotypic correlation it's due to no overlapping rats between the 2 traits.'''
        gcorrfig = pn.pane.PNG(f'{self.path}images/genetic_correlation_matrix{"_sorted" if sorted_gcorr else ""}.png', max_width=1000, max_height=1000, width = 1000, height = 1000)
        gcorrfig = pn.pane.HoloViews(self.make_genetic_correlation_figure(order = 'sorted' if sorted_gcorr else 'cluster' ,save=True), max_width=1000, max_height=1000, width = 1000, height = 1000)
...
The values shown in the table below pass the {self.genome}:{round_version} subjective threshold. 
...
{'* ACI, BN, BUF, F344, M520, MR, WKY, WN: genotypes of founders at the topSNP' if 'ACI' in qtls.columns else ''}'''
...
To control type I error, we estimated the significance threshold by a permutation test, as described in (Cheng and Palmer, 2013).'''
        ...
        eqtlstext = '' if self.genome not in ['rn6', 'rn7'] else f'''## Gene Expression changes:
...
{eqtlstext}'''
        ...
        for index, row in tqdm(list(qtls.iterrows())):
            ...
            try:cau = ann.query('SNP_qtl == @row.TopSNP and trait == @row.trait')[['SNP','Freq','b','-Log10(p)','R2','DP', 'annotation','putative_impact','gene','HGVS.c', 'HGVS.p']].drop_duplicates().sort_values('putative_impact')
            ...
            if phetemp.shape[0]: phetemp = fancy_display(phetemp,f'phewasexactmatch_{row.trait}{row.TopSNP}.csv'.replace(':', '_'))
            ...
            if phewtemp.shape[0]: phewtemp = fancy_display(phewtemp, f'phewas_{row.trait}{row.TopSNP}.csv'.replace(':', '_'))
            ...
            if eqtltemp.shape[0]: eqtltemp = fancy_display(eqtltemp,f'eqtl_{row.trait}{row.TopSNP}.csv'.replace(':', '_'))
...
* We regress out the effect of sex, so the GWAS essentially assumes that alleles have similar effects of the trait in males and females. This means we would not detect an interaction (e.g. an allele that affects a trait only in males). While it is possible to do GWAS for males and females separately, we don’t do this by default because the loss of power makes it hard to interpret the results. If sex differences are of major interest we can discuss ways of examining these.
    ...
    def GeneEnrichment(self, qtls: pd.DataFrame = None, genome: str = 'rn7', padding: int = 2e6, r2thresh: float = .9,
        ...
        gtf['Chr'] = gtf['Chr'].apply(lambda x: self.replaceXYMTtonums(x.split('_')[0])).apply(lambda x: self.replacenumstoXYMT(x)).str.upper()
        ...
        allgenes = gtf.query('biotype == "transcript"')[['gene_id', 'Chr', 'start', 'end']].drop_duplicates().melt(id_vars=['Chr','gene_id']) 
        if select_nearby_genes_by == 'distance':
            ...
            snpgroups['gene_id'] = snpgroups.SNP_origin.apply(lambda x: list(allgenes.query(f'''Chr == "{x.split(':')[0]}" and ({x.split(':')[1]}-{padding}<value<{x.split(':')[1]}+{padding})''').gene_id.unique()))
        ...
        if select_nearby_genes_by == 'r2':
            def get_nearby_genes_r2(rowdata, r2thresh):
                ...
                out = list(allgenes.query(f'Chr == "{self.replacenumstoXYMT(rowdata.Chr).upper()}" and ((variable == "start" and value < @maxbp) or (variable == "end" and value > @minbp))').gene_id.unique())
        ...
        merged_qtls = qtls.groupby('trait')[['gene', 'gene_id_nearby','entrezid_nearby']].agg(sum)
        print('getting entrezid per qtl...')
        merged_qtls['entrezid'] = merged_qtls['gene'].progress_apply(get_entrez_ids)
        print(f'getting entrezid per for all {genome} genes...')
        allentrez = get_entrez_ids(gtf.gene_name.unique())
    ...
    def GeneEnrichment_figure(self, qtls='', merged_qtls='', append_nearby_genes = False):
        ...
        for _, r in qtls.iterrows():
            if not MG.has_node(r['gene'][0]):
                MG.add_node(r['gene'][0], what = 'gene', size = 10 + qtls['gene'].map(lambda x: x[0]).value_counts()[r.gene[0]], color = 'seagreen' )
            if not MG.has_node(r['SNP']):
                MG.add_node(r['SNP'], what = 'topSNP', size = 5 +  qtls['SNP'].value_counts()[r.SNP], color = 'black' )
            if not MG.has_node(r['trait']):
                MG.add_node(r['trait'], what = 'trait', size =15+ qtls['trait'].value_counts()[r.trait], color = 'steelblue' )
            MG.add_edges_from([(r.SNP, r.trait)], weight=r.p, type = 'snp2gene')
            MG.add_edges_from([(r.gene[0], r.trait)], weight=2, type = 'gene2trait')
        ...
        for trait, row in merged_qtls.iterrows():
            ...
            for _, rg in aa.iterrows():
                if not MG.has_node(f'{rg.GO}\n{rg.term}'):
                    MG.add_node(f'{rg.GO}\n{rg.term}' , what = f'Gene Ontology {trait}', size = 20*rg.p_corr, term = rg.term, cls = rg['class'], p = rg.p, color = 'firebrick')
            ...
            for _, rg in bb.iterrows():
                if not MG.has_node(f'{rg.GO}\n{rg.term}'):
                    MG.add_node(f'{rg.GO}\n{rg.term}', what = f'Gene Ontology {trait} with nearby genes', size = max(5,rg.p_corr/5), term = rg.term, cls = rg['class'], p = rg.p, color = 'orange')
    ...
    def project_graph_view(self, append_nearby_genes = False, add_gene_enriched = False, add_phewas= True,
        ...
        for _, r in qtls.iterrows():
            if not MG.has_node(r['gene'][0]):
                MG.add_node(r['gene'][0], what = defaultdict(int, {'gene': 1}), size = 20 + qtls['gene'].map(lambda x: x[0]).value_counts()[r.gene[0]], color = 'seagreen' )
            else:  
            ...
            if not MG.has_node(r['SNP']):
                ...
                MG.nodes[r['gene'][0]]['what']['topsnp'] += 1
                MG.nodes[r['gene'][0]]['size'] *= 2    
            if not MG.has_node(r['trait']):
